...
const SPECIFIC_GOALS = new Set(['exam', 'interview', 'writing', 'relocation', 'career', 'business']);
...
async function generateNarrative({ speakingResults, claudeResults, finalBandLabel, grammarBand, readingBand, locale, apiKey }) {
  const prompt = buildDiagnosisPrompt({ speakingResults, claudeResults, finalBandLabel, grammarBand, readingBand, locale });
...
function buildDiagnosisPrompt({ speakingResults, claudeResults, finalBandLabel, grammarBand, readingBand, locale }) {
  const languageInstruction = locale === 'fr'
    ? 'Write the headline, strength, and blocker in French. The student took the test in French, and the rest of their results page is in French -- do not switch to English.'
    : 'Write the headline, strength, and blocker in English.';
...
async function generateDiagnosis({ speakingResults, claudeResults, finalBandLabel, grammarBand, readingBand, goal, goalDeadlineWeeks, locale, apiKey }) {
  const narrative = await generateNarrative({ speakingResults, claudeResults, finalBandLabel, grammarBand, readingBand, locale, apiKey });
  return {
    ...narrative,
