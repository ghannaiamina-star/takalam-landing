...
// Flags a transcript whose letters are mostly non-Latin script. This is a
// signal that Whisper misdetected the spoken language (e.g. accented English
// transcribed as Arabic), not evidence the student failed to speak English --
// callers must exclude a flagged transcript from grading rather than let it
// be graded as a speaking failure.
function isPredominantlyNonLatinScript(text) {
  const letters = String(text || '').match(/\p{L}/gu) || [];
  if (letters.length < 6) return false; // too short to judge reliably
